...
function preload() {
	...
	game.load.image('wizball', 'wizball.png');
    game.load.image('blockDonut', 'blockDonut.png')

	game.load.physics('physicsData', 'spritesPhaserExample.json');
...
function create() {
    ...
    blockDonut = game.add.sprite(250, 200, 'blockDonut');

	//	Enable the physics bodies on all the sprites and turn on the visual debugger
	game.physics.p2.enable([ contra, bunny, block, wizball, blockDonut ], true);
    ...
    blockDonut.body.clearShapes();
    blockDonut.body.loadPolygon('physicsData', 'BoxHole');
...
function click(pointer) {
	...
	var bodies = game.physics.p2.hitTest(pointer.position, [ contra, bunny, block, wizball, blockDonut ]);
